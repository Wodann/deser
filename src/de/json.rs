...
use super::{DeserializationError, Deserialize, Expected, Unexpected, Visitor};

pub enum Error<VisitorErrorT> {
    Io(std::io::Error),
    Parser {
        line: usize,
        column: usize,
...
pub enum ParserError {
    ...
    LoneTrailingSurrogateInHexEscape,
    UnexpectedEndOfJson,
    InvalidSyntax,
...
pub type Result<T, VisitorErrorT> = std::result::Result<T, Error<VisitorErrorT>>;
...
impl Deserializer {
    ...
    pub fn new<R: ToString>(read: R) -> Self {
        Deserializer {
            read: read.to_string(),
            scratch: Vec::new(),
            remaining_depth: 128,
...
impl ParserNumber {
    fn visit<VisitorT>(self, visitor: VisitorT) -> Result<VisitorT::Value, VisitorT::Error>
    where
        VisitorT: Visitor,
    {
        match self {
    ...
    fn invalid_type<ErrorT: DeserializationError>(self, exp: &dyn Expected) -> ErrorT {
        match self {
            ParserNumber::F64(x) => ErrorT::invalid_type(Unexpected::Float(x), exp),
            ParserNumber::U64(x) => ErrorT::invalid_type(Unexpected::Unsigned(x), exp),
            ParserNumber::I64(x) => ErrorT::invalid_type(Unexpected::Signed(x), exp),
        }
    }
}
...
impl Deserializer {
    ...
    pub fn end(&mut self) -> Result<()> {
        match self.parse_whitespace()? {
            Some(_) => Err(self.peek_error(ParserError::TrailingCharacters)),
            None => Ok(()),
        }
    }

    /// Turn a JSON deserializer into an iterator over values of type T.
    pub fn into_iter<T>(self) -> StreamDeserializer<T>
    where
        T: Deserialize,
    {
    ...
    #[cold]
    fn error(&self, reason: ParserError) -> Error {
    ...
    #[cold]
    fn peek_error(&self, reason: ParserError) -> Error {
    ...
    fn peek_invalid_type(&mut self, exp: &dyn Expected) -> Error {
        let err = match self.peek_or_null().unwrap_or(b'\x00') {
            ...
            _ => self.peek_error(ParserError::ExpectedSomeValue),
        };
        ...
        V: de::Visitor<'any>,
    {
        let peek = match tri!(self.parse_whitespace()) {
            Some(b) => b,
            None => {
                return Err(self.peek_error(ParserError::EofWhileParsingValue));
            }
        };
        ...
        V: de::Visitor<'any>,
    {
        ...
        let value = match buf.parse() {
            ...
            Err(_) => {
                return Err(self.error(ParserError::NumberOutOfRange));
            }
        };
        ...
        V: de::Visitor<'any>,
    {
        match tri!(self.parse_whitespace()) {
            Some(b'-') => {
                return Err(self.peek_error(ParserError::NumberOutOfRange));
            }
            Some(_) => {}
            None => {
                return Err(self.peek_error(ParserError::EofWhileParsingValue));
        ...
        match tri!(self.next_char_or_null()) {
            b'0' => {
                ...
                match tri!(self.peek_or_null()) {
                    b'0'..=b'9' => Err(self.peek_error(ParserError::InvalidNumber)),
                    _ => Ok(()),
                }
            }
            c @ b'1'..=b'9' => {
            ...
            _ => Err(self.error(ParserError::InvalidNumber)),
        }
    }
    ...
    fn parse_ident(&mut self, ident: &[u8]) -> Result<()> {
        for expected in ident {
            match tri!(self.next_char()) {
                None => {
                    return Err(self.error(ParserError::EofWhileParsingValue));
                }
                Some(next) => {
                    if next != *expected {
                        return Err(self.error(ParserError::ExpectedSomeIdent));
                    }
                }
            }
        }
    ...
    fn parse_decimal(
        ...
        if exponent_after_decimal_point == 0 {
            match tri!(self.peek()) {
                Some(_) => return Err(self.peek_error(ParserError::InvalidNumber)),
                None => return Err(self.peek_error(ParserError::EofWhileParsingValue)),
            }
        }
    ...
    fn parse_exponent(
        ...
        let mut exp = match next {
            ...
            _ => {
                return Err(self.error(ParserError::InvalidNumber));
            }
        };
    ...
    fn f64_from_parts(&mut self, positive: bool, significand: u64, exponent: i32) -> Result<f64> {
        ...
        if f.is_infinite() {
            Err(self.error(ParserError::NumberOutOfRange))
        } else {
    ...
    fn parse_any_signed_number(&mut self) -> Result<ParserNumber> {
        ...
        let value = match peek {
            ...
            _ => Err(self.peek_error(ParserError::InvalidNumber)),
        };

        let value = match tri!(self.peek()) {
            Some(_) => Err(self.peek_error(ParserError::InvalidNumber)),
            None => value,
        };
    ...
    fn scan_or_eof(&mut self, buf: &mut String) -> Result<u8> {
        match tri!(self.next_char()) {
            ...
            None => Err(self.error(ParserError::EofWhileParsingValue)),
        }
    }
    ...
    fn parse_object_colon(&mut self) -> Result<()> {
        match tri!(self.parse_whitespace()) {
            ...
            Some(_) => Err(self.peek_error(ParserError::ExpectedColon)),
            None => Err(self.peek_error(ParserError::EofWhileParsingObject)),
        }
    }
    ...
    fn end_seq(&mut self) -> Result<()> {
        match tri!(self.parse_whitespace()) {
            ...
            Some(b',') => {
                self.eat_char();
                match self.parse_whitespace() {
                    Ok(Some(b']')) => Err(self.peek_error(ParserError::TrailingComma)),
                    _ => Err(self.peek_error(ParserError::TrailingCharacters)),
                }
            }
            Some(_) => Err(self.peek_error(ParserError::TrailingCharacters)),
            None => Err(self.peek_error(ParserError::EofWhileParsingList)),
        }
    }
    ...
    fn end_map(&mut self) -> Result<()> {
        match tri!(self.parse_whitespace()) {
            ...
            Some(b',') => Err(self.peek_error(ParserError::TrailingComma)),
            Some(_) => Err(self.peek_error(ParserError::TrailingCharacters)),
            None => Err(self.peek_error(ParserError::EofWhileParsingObject)),
        }
    }
    ...
    fn ignore_value(&mut self) -> Result<()> {
        ...
        loop {
            ...
            let frame = match peek {
                ...
                _ => return Err(self.peek_error(ParserError::ExpectedSomeValue)),
            };
            ...
            loop {
                match tri!(self.parse_whitespace()) {
                    ...
                    Some(_) => {
                        if accept_comma {
                            return Err(self.peek_error(match frame {
                                b'[' => ParserError::ExpectedListCommaOrEnd,
                                b'{' => ParserError::ExpectedObjectCommaOrEnd,
                                _ => unreachable!(),
                            }));
                        } else {
                            break;
                    ...
                    None => {
                        return Err(self.peek_error(match frame {
                            b'[' => ParserError::EofWhileParsingList,
                            b'{' => ParserError::EofWhileParsingObject,
                            _ => unreachable!(),
                        }));
                    }
                }
            ...
            if frame == b'{' {
                match tri!(self.parse_whitespace()) {
                    ...
                    Some(_) => return Err(self.peek_error(ParserError::KeyMustBeAString)),
                    None => return Err(self.peek_error(ParserError::EofWhileParsingObject)),
                }
                tri!(self.read.ignore_str());
                match tri!(self.parse_whitespace()) {
                    Some(b':') => self.eat_char(),
                    Some(_) => return Err(self.peek_error(ParserError::ExpectedColon)),
                    None => return Err(self.peek_error(ParserError::EofWhileParsingObject)),
                }
            }
        ...
        match tri!(self.next_char_or_null()) {
            b'0' => {
                ...
                if let b'0'..=b'9' = tri!(self.peek_or_null()) {
                    return Err(self.peek_error(ParserError::InvalidNumber));
                }
            }
            b'1'..=b'9' => {
                while let b'0'..=b'9' = tri!(self.peek_or_null()) {
    ...
    ($this:ident $($body:tt)*) => {
        if_checking_recursion_limit! {
            ...
            if $this.remaining_depth == 0 {
                return Err($this.peek_error(ParserError::RecursionLimitExceeded));
            }
        }
...
impl<'de, 'a, R: Read<'de>> de::Deserializer<'de> for &'a mut Deserializer<R> {
        ...
        V: Visitor,
    {
        ...
        let value = match peek {
            ...
            _ => Err(self.peek_error(ParserError::ExpectedSomeValue)),
        };
        ...
        match tri!(self.parse_whitespace()) {
            Some(b'{') => {
                ...
                match tri!(self.parse_whitespace()) {
                    ...
                    Some(_) => Err(self.error(ParserError::ExpectedSomeValue)),
                    None => Err(self.error(ParserError::EofWhileParsingObject)),
                }
            }
            Some(b'"') => visitor.visit_enum(UnitVariantAccess::new(self)),
            Some(_) => Err(self.peek_error(ParserError::ExpectedSomeValue)),
            None => Err(self.peek_error(ParserError::EofWhileParsingValue)),
        }
    }
        ...
        let peek = match tri!(self.de.parse_whitespace()) {
            ...
            Some(b) => {
                if self.first {
                    ...
                    return Err(self.de.peek_error(ParserError::ExpectedListCommaOrEnd));
                }
            }
            None => {
                return Err(self.de.peek_error(ParserError::EofWhileParsingList));
            }
        };

        match peek {
            Some(b']') => Err(self.de.peek_error(ParserError::TrailingComma)),
            Some(_) => Ok(Some(tri!(seed.deserialize(&mut *self.de)))),
            None => Err(self.de.peek_error(ParserError::EofWhileParsingValue)),
        }
    }
}
...
impl<'de, 'a, R: Read<'de> + 'a> de::MapAccess<'de> for MapAccess<'a, R> {
        ...
        K: de::DeserializeSeed<'de>,
    {
        let peek = match tri!(self.de.parse_whitespace()) {
            ...
            Some(b) => {
                if self.first {
                    ...
                    return Err(self.de.peek_error(ParserError::ExpectedObjectCommaOrEnd));
                }
            }
            None => {
                return Err(self.de.peek_error(ParserError::EofWhileParsingObject));
            }
        };
        ...
        match peek {
            ...
            Some(b'}') => Err(self.de.peek_error(ParserError::TrailingComma)),
            Some(_) => Err(self.de.peek_error(ParserError::KeyMustBeAString)),
            None => Err(self.de.peek_error(ParserError::EofWhileParsingValue)),
        }
    }
    ...
    ($method:ident, $delegate:ident) => {
            ...
            V: Visitor,
        {
            ...
            match tri!(self.de.peek()) {
                Some(b'0'..=b'9' | b'-') => {}
                _ => return Err(self.de.error(ParserError::ExpectedNumericKey)),
            }
            ...
            match tri!(self.de.peek()) {
                ...
                _ => return Err(self.de.peek_error(ParserError::ExpectedDoubleQuote)),
            }
    ...
    R: Read<'de>,
{
        ...
        V: Visitor,
    {
        ...
        let peek = match tri!(self.de.next_char()) {
            Some(b) => b,
            None => {
                return Err(self.de.peek_error(ParserError::EofWhileParsingValue));
            }
        };
